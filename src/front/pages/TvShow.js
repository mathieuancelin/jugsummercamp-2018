            ...
            <div className="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
              {seasons.map((s, idx) => (
                <div className="panel panel-default" key={`season-${s.number}`}>
                  ...
                  <div
                    id={`collapse-${s.number}`}
                    className={
                      idx === expandId ? 'panel-collapse collapse in' : 'panel-collapse collapse'
                    }
                    role="tabpanel"
                    aria-labelledby={`heading-${s.number}`}>
                            ...
                            <tr key={`episode-${e.id}`}>
                              ...
                              <td>
                                {!e.watched && (
                                  <button
                                    type="button"
                                    className="btn addBigBtn"
                                    onClick={this.markEpisodeWatched(e.id, true)}>
                                    {' '}
                                    ADD{' '}
                                  </button>
                                )}
                                {e.watched && (
                                  <button
                                    type="button"
                                    className="btn addBigBtn"
                                    onClick={this.markEpisodeWatched(e.id, false)}>
                                    <i className="glyphicon glyphicon-ok" />
                                  </button>
                                )}
                              </td>
